...
**Odds (шансы):**
$$
\text{Odds} = \frac{P(y=1)}{P(y=0)} = e^{\mathbf{w}^T \mathbf{x} + b}
$$
...
- **L2 (Ridge)**: penalty = $\lambda \|\mathbf{w}\|^2_2$
- **L1 (Lasso)**: penalty = $\lambda \|\mathbf{w}\|_1$
...
**Когда применять нельзя:**
- ❌ Очень большие данные ($O(n^3)$ для обучения → используйте linear SVM или LogReg)
...
**Цель:**  
Найти гиперплоскость с **максимальным margin**.
