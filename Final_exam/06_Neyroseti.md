...
> **Зачем этот модуль?**  
> Нейронные сети — это **универсальные аппроксиматоры**. Теоретически они могут выучить *любую* функцию, какой бы сложной она ни была.
> - **Гибкость**: от распознавания лиц до управления беспилотниками — всё это нейросети.
> - **End-to-End**: сети сами извлекают признаки из данных (Feature Extraction), избавляя нас от ручного проектирования фичей.
> - **Масштабируемость**: чем больше данных и вычислительных мощностей (GPU), тем умнее становится сеть.
...
- **Эффективное вычисление градиентов** для всех параметров за $O(\text{параметры})$
...
**Прямой проход (Forward Pass) для слоя:**
$$
...
**Градиент для выходного слоя (MSE):**
$$
\frac{\partial L}{\partial \hat{y}} = -(\mathbf{y} - \hat{\mathbf{y}})
$$

**Градиент для весов скрытого слоя (через градиент следующего):**
$$
\frac{\partial L}{\partial \mathbf{W}_1} = \underbrace{\frac{\partial L}{\partial \mathbf{h}_2}}_{\text{ошибка свыше}} \cdot \underbrace{\sigma'(\mathbf{z}_1)}_{\text{активация}} \cdot \underbrace{\mathbf{x}^T}_{\text{вход}}
$$

> [!CAUTION]
> **Production Warning: Vanishing Gradients (Затухание градиентов)**  
> Если вы используете `Sigmoid` в сети глубже 5 слоев, градиенты при умножении $0.25 \cdot 0.25 \dots$ станут настолько малыми ($\approx 0$), что веса первых слоев перестанут обновляться. Модель перестанет учиться. **Решение**: всегда используйте `ReLU` для скрытых слоев и `He initialization`.

> [!TIP]
> **Аналогия: Сломанный телефон с учителем**  
> Представьте цепочку людей, передающих сообщение. Последний говорит результат учителю. Учитель говорит: "Ошибка — 10 баллов". Последний человек понимает свою вину и говорит предпоследнему: "Ты мне передал слишком громко, поэтому я ошибся на 5 баллов". Предпоследний корректирует себя и передает вину дальше. Так "вина" (градиент) доходит до самого первого человека.
...
**Последствия:**
- Ранние слои **почти не обучаются** (градиенты $\approx 0$)
...
3. **Residual connections** (ResNet): $\mathbf{y} = F(\mathbf{x}) + \mathbf{x}$ (skip connections)
...
**3. Softmax (для мультиклассификации):**
$$
...
> [!IMPORTANT]
> **Dead ReLU Problem**  
> Если веса инициализированы так, что $\mathbf{W}\mathbf{x} + \mathbf{b}$ всегда меньше нуля, нейрон выдает 0 и имеет градиент 0. Он "умирает" и больше никогда не обновится. Если у вас 50% сети выдает нули — проверьте инициализацию весов или попробуйте `Leaky ReLU` ($\max(0.01z, z)$).

> [!TIP]
> **Аналогия: Биологический нейрон**  
> Функция активации — это порог срабатывания. Если сигнал от других нейронов слабый, нейрон "молчит" (0). Как только сигнал превышает порог, нейрон "стреляет" импульсом в следующий слой. ReLU — самая простая и эффективная имитация этого процесса.
### 4) Middle-level нюансы
...
class SimpleNet(nn.Module):
    def __init__(self, input_size, hidden_size, num_classes):
        ...
        self.fc1 = nn.Linear(input_size, hidden_size)  # Полный слой 1
    ...
    def forward(self, x):
        ...
        out = self.relu(out)
        out = self.fc2(out)
        return out  # Без Softmax, т.к. CrossEntropyLoss делает это внутри!
...
# 4. Проход обучения (одна итерация)
dummy_data = torch.randn(1, 10)         # batch_size=1
dummy_target = torch.tensor([1])        # Правильный класс
...
# B. Backward pass
optimizer.zero_grad()  # Обнулить старые градиенты! КРИТИЧНО!
loss.backward()        # Вычислить новые градиенты (backprop)
optimizer.step()       # Обновить веса (w = w - lr * grad)

print(f"Loss: {loss.item():.4f}")
```

### Таблица: Функции активации

| Активация | Формула | Range | Где использовать | Примечание |
|-----------|---------|-------|------------------|------------|
| **ReLU** | max(0, x) | [0, ∞) | Hidden layers (Default) | Быстрая, нет затухания градиента (для x>0) |
| **Sigmoid** | 1 / (1 + e⁻ˣ) | (0, 1) | Output (Binary Class) | "Вероятность", затухает градиент |
| **Tanh** | (eˣ - e⁻ˣ) / ... | (-1, 1) | RNN, Hidden (редко) | Центрирована в 0 |
| **Softmax** | eˣ / Σeˣ | (0, 1) | Output (Multi-class) | Сумма выходов = 1 |

---

## 🎯 Q&A для экзамена

**Q1: Что такое Backpropagation своими словами?**
> Это способ сказать каждому весу в сети, насколько сильно он виноват в ошибке. Мы считаем градиенты от конца сети (loss) к началу (input), используя Chain Rule, и корректируем веса, чтобы уменьшить ошибку.

**Q2: Почему нужна нелинейность (activation functions)?**
> Без нелинейностей вся нейросеть (сколько бы слоев ни было) схлопнется в одну линейную регрессию (W2 * (W1 * x) = W_new * x). Нелинейности (ReLU, Sigmoid) позволяют аппроксимировать сложные кривые.

**Q3: Что такое Vanishing Gradient и как его решить?**
> При использовании Sigmoid/Tanh в глубоких сетях градиенты становятся очень маленькими при передаче назад (умножение чисел < 1). Первые слои перестают обучаться.
> **Решение:** Использовать ReLU (градиент = 1), Batch Normalization, Residual Connections (ResNet).

**Q4: Почему Adam обычно лучше SGD?**
> SGD просто идет в сторону градиента. Adam (Adaptive Moment Estimation) учитывает:
> 1. Инерцию (Momentum) — продолжает двигаться в том же направлении.
> 2. Адаптивность шага (RMSProp) — делает разные шаги для разных параметров.
> Это делает сходимость быстрее и устойчивее.
