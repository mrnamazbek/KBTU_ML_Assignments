...
Входное изображение: $\mathbf{I}$ ($H \times W$)  
Фильтр (kernel): $\mathbf{K}$ ($k \times k$), обычно $k = 3$ или $5$
...
>
> **Визуализация (Sliding Window):**
> ```
> Вход (I)       Фильтр (K)      Результат
> [ 1 0 1 ]      [ 1 0 ]
> [ 0 1 0 ]  *   [ 0 1 ]  =>  [ 2 1 ]
> [ 1 0 1 ]                   [ 1 2 ]
>    ↑             ↑             ↑
...
**Множество фильтров:**  
Один сверточный слой обычно содержит $N$ фильтров $\to$ $N$ feature maps.

**Output:** $(H_{out}, W_{out}, N)$
...
```
Число параметров = 224 \cdot 224 \cdot 3 \cdot N_{hidden} \approx 150,000 \cdot N_{hidden}
```

При $N_{hidden} = 1000 \to 150$ млн параметров только в первом слое!

**Свертка (фильтр 3×3×3, 64 фильтра):**
```
Число параметров = k \cdot k \cdot C_{in} \cdot C_{out} + C_{out}
```
где:
- $k$ — размер фильтра
- $C_{in}$ — входные каналы
- $C_{out}$ — выходные (число фильтров)

Например, $3 \times 3$, $3 \to 64$:
$3 \cdot 3 \cdot 3 \cdot 64 + 64 = 1792$ параметра.
...
Вместо pooling используем свертку с $\text{stride} = 2 \to$ уменьшение размера без отдельного слоя pooling.
