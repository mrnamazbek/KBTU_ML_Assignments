...
> **Зачем этот модуль?**  
> Основы Machine Learning — это фундамент, на котором построены все современные ML-алгоритмы. Понимание k-NN, линейной регрессии, bias-variance tradeoff и кросс-валидации критично для:
> - **Правильного выбора модели** под конкретную задачу
> - **Диагностики проблем** (переобучение, недообучение, data leakage)
> - **Интерпретации результатов** и объяснения решений модели бизнесу
> 
> Эти концепции встречаются на каждом собеседовании и в каждом production ML-проекте.
...
$$
где $\mathcal{N}_k(\mathbf{x})$ — множество $k$ ближайших соседей точки $\mathbf{x}$.
...
**Взвешенный вариант:**
$$
\hat{y} = \frac{\sum_{i=1}^{k} w_i \cdot y_i}{\sum_{i=1}^{k} w_i}, \quad \text{где} \quad w_i = \frac{1}{d(\mathbf{x}, \mathbf{x}_i)}
$$
Более близкие соседи имеют больший вес.

> [!WARNING]
> **Production предупреждение:** При использовании взвешенного k-NN в production следите за случаем $d(\mathbf{x}, \mathbf{x}_i) = 0$ (точное совпадение с обучающим примером). Это приведёт к делению на ноль! Добавляйте малое $\epsilon$ в знаменатель: $w_i = 1/(d + \epsilon)$.
...
**Пример:**  
В 1D для покрытия 10% пространства нужно 10% данных. В 10D для покрытия 10% объема нужно $10^{1/10} \approx 79\%$ данных по каждой оси!
...
$k = \sqrt{n}$ (где $n$ — размер обучающей выборки), затем подбор через **кросс-валидацию**.

**Проблема 3: Вычислительная сложность**

- **Обучение**: $O(1)$ — просто сохраняем данные
- **Предсказание**: $O(n \cdot d \cdot \log k)$ — для каждого объекта ищем $k$ соседей

**Решение:**
- **KD-Tree, Ball Tree**: $O(d \cdot \log n)$ для поиска соседей (работает плохо при $d > 20$)
...
x' = \frac{x - \mu}{\sigma}
$$
Приводит к нормальному распределению $\mathcal{N}(0, 1)$. Используйте, когда данные не ограничены фиксированным диапазоном.

**Нормализация (MinMaxScaler):**
$$
x' = \frac{x - x_{\min}}{x_{\max} - x_{\min}} \in [0, 1]
$$
Используйте при известных границах значений (например, изображения [0, 255] → [0, 1]).

> [!CAUTION]
> **Data Leakage Alert!** Не применяйте `fit()` scaler на всех данных сразу! В кросс-валидации scaler должен обучаться ТОЛЬКО на train фолде:
> ```python
> # ❌ НЕПРАВИЛЬНО:
...
> cross_val_score(pipeline, X, y, cv=5)  # fit только на train
> ```
...
- $\mathbf{X} \in \mathbb{R}^{n \times p}$ — матрица признаков (design matrix)
- $\boldsymbol{\beta} \in \mathbb{R}^p$ — вектор коэффициентов
- $\boldsymbol{\epsilon} \sim \mathcal{N}(0, \sigma^2 \mathbf{I})$ — ошибки (Gaussian noise)

**Функция потерь (MSE - Mean Squared Error):**
$$
L(\boldsymbol{\beta}) = \|\mathbf{y} - \mathbf{X}\boldsymbol{\beta}\|^2 = \sum_{i=1}^{n}(y_i - \mathbf{x}_i^T \boldsymbol{\beta})^2
$$
...
> [!IMPORTANT]
> **Условие применимости:** Матрица $\mathbf{X}^T \mathbf{X}$ должна быть **обратимой** (полного ранга $p$). Если $\text{rank}(\mathbf{X}) < p$ → **мультиколлинеарность**, используйте Ridge регуляризацию.
...
Где $R^2 \in [0, 1]$: чем ближе к 1, тем лучше модель объясняет дисперсию данных.
...
|-------|-----------|-------------------|
| **Нормальное уравнение** | $O(p^3 + np^2)$ | Малое $p$ ($p < 10,000$), нужно точное решение |
| **Градиентный спуск** | $O(\text{iterations} \cdot np)$ | Большое $p$, онлайн обучение, регуляризация |
...
где градиент:
$$
\nabla L(\boldsymbol{\beta}) = -2\mathbf{X}^T(\mathbf{y} - \mathbf{X}\boldsymbol{\beta}) = -2\mathbf{X}^T \mathbf{e}
$$
...
**Последствия:**
- Матрица $\mathbf{X}^T \mathbf{X}$ близка к вырожденной → **неустойчивость** $(\mathbf{X}^T \mathbf{X})^{-1}$
- Коэффициенты имеют **огромные значения** и **высокую дисперсию**
- Малое изменение данных → сильное изменение $\boldsymbol{\beta}$

**Диагностика: VIF (Variance Inflation Factor)**
$$
\text{VIF}_j = \frac{1}{1 - R^2_j}
$$

где $R^2_j$ — коэффициент детерминации регрессии $j$-го признака на остальные.
...
> [!WARNING]
> **Production предупреждение:** В реальных данных VIF > 100 встречается регулярно! Например, если добавили признаки `price` и `price_log` одновременно. Всегда проверяйте VIF после feature engineering.
...
Пусть $\hat{y}(\mathbf{x})$ — предсказание модели, $y$ — истинное значение, $f(\mathbf{x})$ — истинная зависимость.
...
$$
\text{Bias}^2 = \left(\mathbb{E}[\hat{y}(\mathbf{x})] - f(\mathbf{x})\right)^2
$$
...
$$
\text{Variance} = \mathbb{E}\left[\left(\hat{y}(\mathbf{x}) - \mathbb{E}[\hat{y}(\mathbf{x})]\right)^2\right]
$$
...
$$
\sigma^2 = \mathbb{E}[(y - f(\mathbf{x}))^2]
$$

Шум в данных, который невозможно смоделировать.

**Полная формула (разложение MSE):**
$$
\begin{aligned}
\text{MSE} &= \mathbb{E}[(y - \hat{y})^2] \\
&= \underbrace{\left(\mathbb{E}[\hat{y}] - f\right)^2}_{\text{Bias}^2} + \underbrace{\mathbb{E}\left[\left(\hat{y} - \mathbb{E}[\hat{y}]\right)^2\right]}_{\text{Variance}} + \underbrace{\sigma^2}_{\text{Irreducible}}
\end{aligned}
$$
