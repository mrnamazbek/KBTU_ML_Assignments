...
> **Зачем этот модуль?**  
> Ансамбли — это **State-of-the-art** для табличных данных. Если вы работаете с Excel-подобными данными (не картинки/текст), ансамбли — ваш главный инструмент.
> - **Точность**: один в поле не воин. Сотня "слабых" моделей почти всегда побеждает одну "сильную".
> - **Устойчивость**: усреднение мнений разных моделей защищает от случайных выбросов и переобучения.
> - **SOTA**: победные решения на Kaggle в 90% случаев используют XGBoost, LightGBM или CatBoost.
...
1. Для $b = 1$ до $B$ (число моделей):
   - Сформировать **Bootstrap sample** $D_b$ (выборка $n$ объектов с возвращением).
...
$$
\hat{y} = \text{mode}\{f_b(\mathbf{x})\} \quad \text{(голосование большинства)}
$$

**Out-of-Bag (OOB):**  
Вероятность того, что объект **не попадет** в bootstrap sample при $n \to \infty$:
$$
...
> Один врач может ошибиться из-за усталости или субъективного опыта. Если собрать 100 врачей и попросить каждого поставить диагноз, основываясь на *разных* анализах пациента, и затем проголосовать — вероятность ошибки упадет в разы. Это и есть Bagging.

> [!CAUTION]
> **Production Warning: OOB Error vs Cross-Validation**  
> OOB оценка в Random Forest — отличный способ быстрой проверки. Но если у вас в данных есть **временная зависимость** (Time Series), OOB будет давать слишком оптимистичный результат (leakage). В этом случае используйте только `TimeSeriesSplit`.
...
1. Сформировать bootstrap выборку $D_t \subset D$.
2. Построить дерево: на каждом шаге выбирать лучший сплит только из **m случайных признаков** (обычно $m = \sqrt{p}$).

**Предсказание (soft voting):**
$$
...
> - `n_estimators`: Число деревьев. Больше = лучше (до плато). Не переобучает модель!
> - `max_features`: Главный рычаг управления. Чем меньше, тем более разнообразны (декоррелированы) деревья.
> - `max_depth`: Глубина. Для RF обычно ставят `None` или очень глубокие деревья, так как ансамбль борется с их переобучением.

> [!TIP]
> **Аналогия: Суд присяжных с узкой специализацией**  
> Представьте, что в суде 12 присяжных, но каждому разрешили смотреть только на 3 случайные улики из 10. Если каждый вынесет решение, и мы их усредним, итоговый вердикт будет гораздо объективнее, чем если бы все 12 смотрели на одну и ту же самую яркую (но, возможно, обманчивую) улику.
...
1. Инициализация константой: $F_0(\mathbf{x}) = \arg\min_\gamma \sum L(y_i, \gamma)$.
2. Для $m = 1 \dots M$:
   - Вычислить антиградиент (остатки): $r_{mi} = -\left[\frac{\partial L(y_i, F(\mathbf{x}_i))}{\partial F(\mathbf{x}_i)}\right]_{F=F_{m-1}}$.
   - Обучить "слабое" дерево $h_m$ предсказывать эти остатки.
   - Обновить модель: $F_m = F_{m-1} + \eta \cdot h_m(\mathbf{x})$.
...
> В бустинге `learning_rate` ($\eta$) и `n_estimators` ($M$) связаны обратной зависимостью. Если вы уменьшаете шаг (для точности), вы ОБЯЗАНЫ увеличить количество деревьев, иначе модель не успеет дойти до минимума. И наоборот — высокий шаг быстро ведет к переобучению.

> [!TIP]
> **Аналогия: Скульптор и мелкая наждачка**  
> Первая модель в бустинге — это грубая обработка камня (большой bias). Каждая следующая модель — это все более мелкая наждачная бумага, которая убирает шероховатости (ошибки) предыдущего этапа. В конце мы получаем идеально гладкую фигуру. Но если переборщить с количеством проходов наждачкой (слишком много итераций), можно стереть саму фигуру (overfitting).
